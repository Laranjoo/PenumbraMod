...
namespace PenumbraMod.Content.Resources
{
	public class CosmicBeetLifeOverlay : ModResourceOverlay
	{
		...
		public override void PostDrawResource(ResourceOverlayDrawContext context) {
			...
			int exampleFruits = Main.LocalPlayer.GetModPlayer<CosmicBeetPlayer>().cosmicbeet;

			// Life resources are drawn over in groups of one
		...
		private void DrawClassicFancyOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = heartTexture ??= ModContent.Request<Texture2D>("PenumbraMod/Content/Resources/CosmicBeetHeart");
			context.Draw();
		}
		...
		private void DrawFancyPanelOverlay(ResourceOverlayDrawContext context) {
			...
			// "context" contains information used to draw the resource
			// If you want to draw directly on top of the vanilla hearts, just replace the texture and have the context draw the new texture
			context.texture = fancyPanelTexture ??= ModContent.Request<Texture2D>("PenumbraMod/Content/Resources/CosmicBeetHeart_Panel");
		...
		private void DrawBarsOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = barsFillingTexture ??= ModContent.Request<Texture2D>("PenumbraMod/Content/Resources/CosmicBeetBar_Fill");
			context.Draw();
		}
		...
		private void DrawBarsPanelOverlay(ResourceOverlayDrawContext context) {
			...
			context.texture = barsPanelTexture ??= ModContent.Request<Texture2D>("PenumbraMod/Content/Resources/CosmicBeetBar_Panel");
